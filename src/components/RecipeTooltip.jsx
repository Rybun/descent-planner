...
export default function RecipeTooltip({ recipeId, children }) {
  ...
  const rawName    = part ? getName(part, lang) : itemId;
  ...
  const rawDesc = WEAPON_PART_DESCS[itemId]?.[lang] || '';
  ...
  const conflictRawDesc = conflictId ? (WEAPON_PART_DESCS[conflictId]?.[lang] || '') : '';
  const conflictDesc    = conflictRawDesc.replace(/^"|"$/g, '').trim();
  const conflictNodes   = conflictDesc ? renderNodes(parseGameText(conflictDesc)) : null;
      ...
      {visible && (
        <span className="rtt-bubble" style={{ left: offsetX, top: offsetY }}>
          ...
          <span className="rtt-header">
            ...
            {equipped && (
              <span className="rtt-equipped-badge">{t('shop.alreadyEquipped')}</span>
            )}
            {conflictId && (
              <span className="rtt-conflict-badge">!</span>
            )}
          </span>
          ...
          {conflictId && (
            <span className="rtt-conflict">
              ...
              {conflictNodes && (
                <span className="rtt-conflict-effect">{conflictNodes}</span>
              )}
            </span>
          )}
